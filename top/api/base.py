...
SYSTEM_GENERATE_VERSION = "taobao-sdk-python-20160409"
...
class RestApi(object):
    ...
    def getResponse(self, authrize=None, timeout=30):
        ...
        sign_parameter['sign'] = sys_parameters['sign']
        resp = requests.get(self.__domain, params=sign_parameter)
        if resp.status_code is not 200:
            raise RequestException('invalid http status ' + str(resp.status_code) + ',detail body:' + resp.text)
        result = resp.text
        json_obj = json.loads(result)
        if "error_response" in json_obj:
            error = TopException()
            if P_CODE in json_obj["error_response"]:
                error.errorcode = json_obj["error_response"][P_CODE]
            if P_MSG in json_obj["error_response"]:
                error.message = json_obj["error_response"][P_MSG]
            if P_SUB_CODE in json_obj["error_response"]:
                error.subcode = json_obj["error_response"][P_SUB_CODE]
            if P_SUB_MSG in json_obj["error_response"]:
                error.submsg = json_obj["error_response"][P_SUB_MSG]
            error.application_host = resp.headers.get("Application-Host", "")
            error.service_host = resp.headers.get("Location-Host", "")
            raise error
        return json_obj
